...
member.send(`Whats up! ${member} Welcome to ${member.guild.name}! 😃`)
...
if(!message.member.hasPermission("ADMINISTRATOR")) return message.channelsend("No can do pal!")
...
if(!message.member.hasPermission("MANAGE_MEMBERS")) return message.channelsend("No can do pal!")
...
bot.on("message", async message => {
  ...
  if(rMember.roles.has(Muted.id)) return message.reply("They already have that role.");
...
if(cmd === `${prefix}unmute`){
  ...
  if(!rMember.roles.has(Muted.id)) return message.reply("They don't have that role.");
...
if(wUser.hasPermission("ADMINISTRATOR")) return message.reply("You can't warn a moderator");
if(!message.member.hasPermission("ADMINISTRATOR")) return message.reply("Sorry pal, you can't do that.");
